...
export const generateCampaignCore = ai.defineFlow(
  ...
  async (input) => {
    const prompt = `You are a master storyteller and game designer creating the foundation for a new tabletop RPG campaign. Based on the provided setting, tone, and characters, generate the core narrative concepts.
...
export const generateCampaignFactions = ai.defineFlow(
  ...
  async (input) => {
    const prompt = `You are a master storyteller and game designer. You are designing the active threats and factions for a tabletop RPG campaign.
...
export const generateCampaignNodes = ai.defineFlow(
  ...
  async (input) => {
    const prompt = `You are a master storyteller and game designer creating a web of interconnected situations for a tabletop RPG.
...
## Your Task
Create a web of 5 to 7 interconnected situation nodes (these can be people, places, or problems). For each node:
1.  **title:** A clear, evocative name (e.g., "The Whispering Tombs," "Blackwood's Ambush").
...
6.  **challenges (2-3):** Provide 2-3 general challenges, obstacles, or groups of minions present at this node. Examples: "A group of surly guards", "A treacherous environmental hazard", "A magically sealed door".
7.  **faces (1-2):** Name 1 or 2 key NPCs at this node. For each Face, provide:
    *   **name:** The NPC's name.
    *   **role:** Their job or function (e.g., "Village Elder", "Guard Captain").
    *   **aspect:** A descriptive aspect implying their skills and challenge (e.g., "Battle-Scarred Veteran", "Deceptive Diplomat").
